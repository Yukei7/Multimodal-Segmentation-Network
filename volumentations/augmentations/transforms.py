...
class RotatePseudo2D(DualTransform):
    def __init__(self, axes=(0, 1), limit=(-90, 90), interpolation=3, border_mode='constant', value=0, mask_value=0,
    ...
    def apply(self, img, angle):
        return F.rotate2d(img, angle, axes=self.axes, reshape=False, interpolation=self.interpolation,
                          border_mode=self.border_mode, value=self.value)

    def apply_to_mask(self, mask, angle):
        return F.rotate2d(mask, angle, axes=self.axes, reshape=False, interpolation=0, border_mode=self.border_mode,
...
class RandomBrightness(Transform):
    def __init__(self, factor=0.2, always_apply=False, p=0.5):
        super().__init__(always_apply, p)
        assert 0 <= factor <= 1
        self.factor = float(factor)

    def apply(self, img, bright_factor=1):
        return F.bright_transform(img, bright_factor=bright_factor)

    def get_params(self, **data):
        return {"bright_factor": float(random.uniform(1 - self.factor, 1 + self.factor))}
...
class ElasticTransformPseudo2D(DualTransform):
    ...
    def apply(self, img, random_state=None):
        return F.elastic_transform_pseudo2D(img, self.alpha, self.sigma, self.alpha_affine,
                                            interpolation=cv2.INTER_LINEAR, border_mode=cv2.BORDER_REFLECT_101,
                                            value=None, random_state=random_state, approximate=False)
...
class ElasticTransform(DualTransform):
    def __init__(self, deformation_limits=(0, 0.25), interpolation=3, border_mode='constant', value=0, mask_value=0,
                 always_apply=False, p=0.5):
        super().__init__(always_apply, p)
...
class Rotate(DualTransform):
    def __init__(self, x_limit=(-15, 15), y_limit=(-15, 15), z_limit=(-15, 15), interpolation=3, border_mode='constant',
                 value=0, mask_value=0, always_apply=False, p=0.5):
        super().__init__(always_apply, p)
    ...
    def apply(self, img, x, y, z):
        return F.rotate3d(img, x, y, z, interpolation=self.interpolation, border_mode=self.border_mode,
                          value=self.value)
