...
def random_crop(img, crop_height, crop_width, crop_depth, h_start, w_start, d_start):
    ...
    if height < crop_height or width < crop_width or depth < crop_depth:
        ...
        warn('image size smaller than crop size, pad by default.', UserWarning)
    x1, y1, z1, x2, y2, z2 = get_random_crop_coords(height, width, depth, crop_height, crop_width, crop_depth, h_start,
                                                    w_start, d_start)
    img = img[x1:x2, y1:y2, z1:z2]
    return img
...
def rescale(img, scale, interpolation=1):
    ...
    scale: scalar float
    """
    return skt.rescale(img, scale, order=interpolation, mode='constant', cval=0, clip=True, multichannel=True,
                       anti_aliasing=False)
    """
...
def bright_transform(img, bright_factor):
    """
    :param img:
    :param bright_factor: How much to adjust the brightness. Can be negative. 0 gives a black image,
    1 gives the original image while 2 increases the brightness by a factor of 2.
    """
    bright_factor = float(bright_factor)
    blank_img = np.zeros_like(img)
    return img * bright_factor + blank_img * (1 - bright_factor)


def elastic_transform_pseudo2D(img, alpha, sigma, alpha_affine, interpolation=cv2.INTER_LINEAR,
                               border_mode=cv2.BORDER_REFLECT_101, value=None, random_state=42, approximate=False):
    ...
    for d in range(depth):
        tmp = img[:, :, d]  # [H, W, C]
        tmp = cv2.warpAffine(tmp, M=matrix, dsize=(width, height), flags=interpolation, borderMode=border_mode,
                             borderValue=value)
        res[:, :, d] = tmp
    img = res
    ...
    for d in range(depth):
        tmp = img[:, :, d]  # [H, W, C]
        tmp = cv2.remap(tmp, map1=map_x, map2=map_y, interpolation=interpolation, borderMode=border_mode,
                        borderValue=value)
        res[:, :, d] = tmp
    img = res
